...
For [private docker registry](%currentPath%/installation/kubernetes/helm/#private-docker-registry) with volume mounted secretes, the registry configurations are automatically inferred from the secrets. In addition the `spring.cloud.dataflow.container.registry-configurations` properties allow to configure explicitly multiple container registries like this:
...
Note: you need to create an [Encrypted Password](https://www.jfrog.com/confluence/display/JFROG/Centrally+Secure+Passwords).
