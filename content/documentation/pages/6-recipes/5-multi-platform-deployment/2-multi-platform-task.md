...
To create the `timestamp-task-2` definition, follow the `timestamp-task` instructions above, except that, for the step when the `Create Task` dialog appears, the name will be `timestamp-task-2`.
...
To verify that the pod executed, you can view the result in the task execution page or by launching the following `kubectl` command to view the pods in the `default` namespace:
...
Now, to launch a task in the practice name space:
...
Using Spring Cloud Data Flow's shell, we create a schedule to launch the `timestamp-task` once a minute on the `default` platform.
...
3. To verify that the scheduled application was properly created and being launched by the CronJob, you can view the result by running the following command `task execution list` from the SCDF shell.
   Alternatively, you can run the following `kubectl` command and see the timestamp-task-sched pods start appearing after 1 minute.
...
Establish environment properties for Spring Cloud Data Flow and launch it:
...
2. Setup an endpoint to your local MySql:
...
3. Now, obtain the `cluster ip` for the newly created `mysql-mac` service so that you can use it for launching and scheduling tasks. To do so, run the following command (in this case a MySql instance running on my machine):
...
Add the following command-line argument when launching the task (replacing `<ext db conn>` with the CLUSTER-IP provided by the `kubectl get svc mysql-mac`):
...
To verify the task was launched, select the **Task executions** tab on the left side of the page to see results that look like:
...
In this section, we create two schedules, each on a different platform.
Using Spring Cloud Data Flow's shell, we create a schedule that launches the `timestamp-task` once a minute on the `default` platform.
...
For this exercise, you need access to a shell. Establish environment properties for Spring Cloud Data Flow and launch it:
...
1. When the **Create Task** dialog appears enter the following into the `Name` field: `cf-timestamp`.
1. Click the **CREATE THE TASK** button.
   ![Name the New Task Definition](images/k8-cf-create-timestamp-maven-task-dialog.png)

Now both task definitions are available:  
...
To verify that the application ran, you can view the result in the task execution page or by launching the following `cf apps` command to view the application in the configured org/space:
...
In this section, we create two schedules, each schedule on a different platform.
Using Spring Cloud Data Flow's shell, we create a schedule that launches the `cf-timestamp` task definition once a minute on the `cfzone` Cloud Foundry platform and a `k8-timestamp` task definition on the `default` namespace of a minikube.
...
1. Now verify that the application that was defined by the task definition has been deployed to the Cloud Foundry by running the `cf apps` command on the org/space that you configured:
...
1. Verify that the scheduled `timestamp-task-cf-sched` actually launched by running the `cf job-history timestamp-task-cf-sched` command on the org/space you configured:
...
1. Verify that the schedule was created, run the`task schedule list --platform kzone` command and view the result:
