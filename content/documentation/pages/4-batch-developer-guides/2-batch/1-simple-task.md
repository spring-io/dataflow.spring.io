...
If you do not have an instance of MariaDB available to you, you can follow these instructions to run a MariaDB docker image for this example:
...
2. Start MariaDB by running the following command:
   ...
   ```bash
   docker run --name mariadb  -e MYSQL_ROOT_PASSWORD=password -e MYSQL_DATABASE=task -d mariadb:latest
   ```
