...
We create three Spring Cloud Stream applications that communicate by using Kafka.
...
Behind the scenes, Spring Cloud Stream turns this `Supplier` into a producer.
By default, the supplier is invoked every second.
On each invocation, the supplier `sendEvents` method constructs a `UsageDetail` object.
...
The default producer output binding for the `sendEvents` method is going to be `sendEvents-out-0` (method name followed by the literal `-out-0`, where `0` is the index).
If the application does not set a destination, Spring Cloud Stream uses this same binding name as the output destination (Kafka topic).
...
The first property overrides the default binding name to `output`, and the second one sets the destination on that binding.

#### Building

Now we can build the Usage Detail Sender application.
In the `usage-detail-sender` directory, use the following command to build the project by using Maven:
...
The following listing shows the Maven coordinates for this artifact:
...
By using them, you can simulate the behavior of actual middleware based binders.
...
- The `testUsageDetailSender` test case uses the test binder to receive messages from the output destination to which the supplier publishes messages.

### `UsageCostProcessor` Processor

Either [directly download the generated project from the Spring Initialzr](https://start.spring.io/starter.zip?type=maven-project&language=java&bootVersion=2.1.4.RELEASE&baseDir=usage-cost-processor-kafka&groupId=io.spring.dataflow.sample&artifactId=usage-cost-processor-kafka&name=usage-cost-processor-kafka&description=Demo+project+for+Spring+Boot&packageName=io.spring.dataflow.sample.usagecostprocessor&packaging=jar&javaVersion=1.8&dependencies=kafka&dependencies=cloud-stream&dependencies=actuator&dependencies=web&dependencies=cloud-connectors) or visit the [Spring Initialzr site](https://start.spring.io/) and follow these instructions:
...
However, in our case, as in the producer shown earlier, we do not want these defaults. Rather, we want to make them more descriptive.
We want to use the binding name as `input` and `output` and provide custom destinations for them.
...
By default, the input binding used by Spring Cloud Stream is `process-in-0` (so is the destination name if the application does not override it).
...
The `spring.cloud.stream.function.bindings.process-in-0` property overrides the binding name to `input` and the `spring.cloud.stream.bindings.input.destination` property sets the destination to the `usage-cost` Kafka topic.

You can extend or override many configuration options to achieve the desired runtime behavior when using Apache Kafka as the message broker.
The [Apache Kafka-binder documentation](https://cloud.spring.io/spring-cloud-static/spring-cloud-stream-binder-kafka/current/reference/html/spring-cloud-stream-binder-kafka.html#_configuration_options) lists the Apache Kafka-specific binder configuration properties.

#### Building

Now you can build the Usage Cost Logger application.
...
This section shows how to run the three applications as stand-alone applications in your `local` environment.
