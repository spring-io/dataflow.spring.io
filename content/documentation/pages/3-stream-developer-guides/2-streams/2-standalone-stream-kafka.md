...
1. In the Dependencies text box, type `Kafka` to select the Apache Kafka binder dependency.
...
In this case, we can use some test Apche Kafka `topics` to verify the outbound and inbound messages.
For instance, testing the `Source` application requires the `output` binding to be set a test `topic`on Apache Kafka.
